...
public class SingleFramebuffer extends GlFramebuffer implements InternalIrisFramebuffer {
    ...
    public void unbind() {
        ...
        // 26.2 moved the main render target from Minecraft#getMainRenderTarget to GameRenderer#mainRenderTarget.
        // Iris injects iris$bindFramebuffer onto it via mixin; loom interface injection isn't applied in this
        // non-remap setup, so reach it through Iris' duck interface.
        ((RenderTargetInterface) (Object) Minecraft.getInstance().gameRenderer.mainRenderTarget()).iris$bindFramebuffer();
    }
